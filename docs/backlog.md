...
| D-93 | **Panel vérification Pointage** (`server.js`, `public/pointage.{html,js}`, `tests/pointage-verif.test.js`) — sur **`pointage.html`** (pas index) : pastille header + section **Vérification** (file non clôturés / à valider + journal acteur / **Code OTP** vs **Saisie manuelle**) pour patron/directeur/observateur. Routes `GET /api/pointage/verif/{count,pending,journal}` ; `valider-recap` journalisé ; observateur autorisé à valider/ajuster. Le bouton Pointage d'`index` reste un lien vers la page. | — |
...
- **Clôture OTP (D-91 / D-92 / D-93)** : preuve = `*_code` ; retenues = `*_finale` ; sync `real_*` si paire complète. Après `patron_valide` : patron/directeur/**observateur** peuvent ajuster ; **responsable** non. Panel vérif sur **`pointage.html`** (`GET /api/pointage/verif/*`). Tests : `cloture-otp` + `pointage-verif`. Smoke : `npm run smoke:cloture`.
