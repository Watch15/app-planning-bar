...
- **Clôture OTP (D-91)** : preuve = `debut_valide_code` / `heure_validee_code` (immutables) ; retenues = `*_finale` ; sync `real_*` seulement quand **les deux** retenues existent (`syncRealHoursFromCloture`). `time_validations` est **append-only**. `patron_valide` ne bloque plus `ajuster-heure`. UI manager ≠ tablette : ne pas réafficher les cartes legacy pour patron/directeur. Responsable de soirée = **soirée active seulement** (`getActivePointageDateStr`). Litiges : `GET /api/shifts/:id/time-validations` **avant** la route `:establishmentId/:date`. Smoke : `npm run smoke:cloture`.
