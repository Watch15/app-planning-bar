...
| D-66 | **E-17 — Tableau de bord consolidé multi-établissements pour le directeur** (`server.js` + `script.js`) — nouveau endpoint `GET /api/week-full-multi` qui agrège les shifts de tous les établissements accessibles (directeur → `assigned_establishments`, patron → tous), même format que `/api/week-full` (groupé par date), chaque shift conserve son `establishment_id`. Côté UI : onglet venue supplémentaire **« ★ Tous mes bars »** ajouté à la fin des onglets venue uniquement pour un directeur avec ≥ 2 établissements assignés. Clic → `isMultiVenueMode = true`, vue Semaine + sous-onglet Tableau de bord forcés, fetch sur l'endpoint multi, `weekSummary` dérivé de `weekFullData`. Les pastilles de shifts du Tableau de bord affichent une chip « nom de l'établissement » (9px) sous les horaires ; non cliquables en multi-mode (vue Jour requiert un seul venue). Clic sur vue Jour pendant le multi-mode → bascule automatique sur le premier onglet venue. Patron + directeur mono-établissement = comportement inchangé | (à commit) |
...
- **Dispos `type:'off'` (indispo, D-63)** : une indispo est purement informative — `start_time`/`end_time` = `null`. **Toujours l'exclure** des vues qui supposent un créneau horaire : `/api/dispos/confirmed` (overlay planning) et `/api/dispos/non-affectees` (à recréer) la filtrent déjà ; ne jamais créer de shift à partir d'un off. Côté affichage, tester `dispo.type === 'off'` avant de formater des heures (sinon `NaN`).
