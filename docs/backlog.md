...
| D-110 | **Validation hebdo : périmètre du responsable de soirée** — `validationEstabFilter` passait un compte staff par `assigned_establishments` (champ directeur) → périmètre vide : page Validation sans personne, `sign` → 404 « Aucun shift à signer », donc récap patron bloqué en 409 (« aucun des deux ne fonctionne », 2026-09-18). Périmètre staff = établissements où il est désigné `pointage_resp` sur la semaine à signer. Bouton « Validation » du planning aussi visible au responsable de la semaine **précédente** (celle qu'on signe). Tests dans `week-validation.test.js`. | ✅ |
