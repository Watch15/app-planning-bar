...
| ~~F-09~~ | ~~**Abonnement agenda iCal (Google/Apple/Outlook)**~~ | Staff / Agenda | ✅ Done (D-72) — flux `.ics` par staff, synchro auto sans login, semaines publiées uniquement |
...
| R-03 | Externaliser les `<script>` inline des HTML (`planning.html` ~2150 l, `pointage.html` ~810 l) vers des `.js` servis statiquement (lintables, cachables) — **avant** tout découpage | P3 |
| R-04 | Découper `server.js` (~4250 l, 101 routes) en routers par domaine (auth, shifts, dispos, pointage, calendrier, établissements). Chantier #10, à faire après R-01/02 | P3 |
...
| C-02 | Agenda (D-72) : envisager `PUBLIC_BASE_URL` (env Railway) pour figer le domaine des URLs `.ics` en cas de domaine custom | P3 |
...
- **Tests** : `npm test` (zéro dépendance, `node --test`) — **69 tests, 3 suites** (`tests/utils.test.js` + `tests/shift-hours.test.js` + `tests/week.test.js`). ⚠️ le mode répertoire `node --test tests/` **n'est pas fiable** (échoue selon la version Node) : lister les fichiers explicitement dans `package.json`. Ajouter un test quand on extrait un helper pur, change une règle de date/heure, ou fixe un bug qui pourrait régresser.
