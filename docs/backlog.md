...
| D-93 | **Panel vérification Pointage** (`server.js`, `public/index.html`, `public/script.js`, `public/index-init.js`, `tests/pointage-verif.test.js`) — pastille sur le bouton Pointage (patron/directeur/observateur) = non clôturés (soirée active) + clôturés non `patron_valide` (semaine). Modale type Dispos : onglet **À vérifier** (badges **Code OTP** / **Saisie manuelle**, ajuster / fin manuelle, valider le récap) + onglet **Journal** (acteur nommé + horodatage + source). Routes `GET /api/pointage/verif/{count,pending,journal}` ; `valider-recap` journalisé ; **observateur autorisé** à valider/ajuster ici (exception à `denyObservateurEdit`). CTA « Ouvrir le pointage soir ». | — |
...
| D-93 / PT-16 | Panel vérif index : pastille + file + journal (OTP/manuel), observateur peut valider | ✅ |
...
- **Clôture OTP (D-91 / D-92 / D-93)** : preuve = `debut_valide_code` / `heure_validee_code` (immutables) ; retenues = `*_finale` ; sync `real_*` seulement quand **les deux** retenues existent. `time_validations` append-only. Après `patron_valide` : patron/directeur/**observateur** peuvent `ajuster-heure` ; **responsable** non. Panel vérif index : `GET /api/pointage/verif/{count,pending,journal}` — pastille = non clôturés (soirée active) + à valider (semaine). Source OTP vs manuel exposée (`source_label` / `debut_source` / `cloture_source`). Observateur **peut** valider le récap ici (exception `denyObservateurEdit`). Litiges shift : route `time-validations` **avant** `:establishmentId/:date`. Tests : `cloture-otp` + `pointage-verif`. Smoke : `npm run smoke:cloture`.
