...
| D-95 | **Onglet Simulation Performance** (`lib/utils.js`, `server.js`, `public/performance.{html,js}`, tests) — calcul **à la volée** (ne touche pas `daily_revenue`) : périmètre **jour ou semaine** (segmented control + nav ‹ ›) ; CA hypo sur jours sans CA réel ; masse **hybride** (shifts pointés en réel + reste en heures planifiées) ; jokers via taux/forfait manuel ou **moyenne/médiane** des `hourly_rate` staff (filtres établissements + groupes). `POST /api/performance/simulate`. | — |
...
- **Simulation Performance (D-95)** : onglet distinct ; calcul éphémère ; hybride réel pointé + planifié ; jokers via manuel ou mean/median (`deriveStaffHourlyStat` / `buildPerformanceSimulation`). Ne jamais écrire `daily_revenue` depuis la simu.
