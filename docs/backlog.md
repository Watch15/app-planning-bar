...
| D-106 | **Refonte Pointage — dates / modes / suppression patron** — à traiter. (1) Deux dates désynchronisées : `#date-select`/`today` (legacy + CA) vs `_cloturesDay` (nav ←→ clôture) — les flèches clôture ne recalent pas le sélecteur ni le legacy. (2) Ancien mode cartes `real_*` (`#legacy-pointage-block`) masqué pour patron/directeur et non lié au panneau clôture OTP (deux sources d'écriture `real_*` sans UI unifiée). (3) Suppression shift côté patron insuffisante : bouton absent si déjà pointé + `DELETE …/pointage` refuse en 409 dès qu'il y a des `real_*` — besoin d'effacer une erreur (extra / mauvais shift) après saisie. | ⏳ |
