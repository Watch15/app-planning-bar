...
| D-106 | **Refonte Pointage — dates / modes / suppression patron** — (1) Date unique : `setActiveDate` cale `today` + `_cloturesDay` ; flèches ←→ appellent `navigatePointageDay` → même jour pour sélecteur, CA, code OTP, clôture et cartes. (2) Modes liés : panneau clôture OTP **et** saisie directe (`#legacy-pointage-block`) visibles ensemble pour les managers ; refresh croisé via `refreshPointageDay`. (3) Patron/directeur peuvent `DELETE …/pointage` même si `real_*` déjà saisis (tablette reste en 409) ; bouton Supprimer sur cartes + lignes clôture. Tests `pointage-delete.test.js`. | ✅ |
