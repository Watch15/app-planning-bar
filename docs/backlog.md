...
| F-05 | **Échange de shifts avec validation patron** | Shifts / Notifications | ✅ **Réactivée le 2026-09-03 (D-90)** pour être présentée au client — active sur `dev` et sur l'instance de démo, **pas** en production client. Le décommentage n'a pas suffi : la feature dormait depuis mai 2026, B2-b (semaines non publiées) et la carte du jour « tapable » ont changé le terrain sous elle. Cf. D-90 |
...
- ~~**F-05 (échange de shifts)** — le code est écrit et testé, **désactivé en attente de ta
  validation**. Coût de mise en service ≈ retirer des commentaires.~~ → **Réactivée D-90** (dev/démo).
...
**Retrait de la jokerisation : tenté puis annulé (2026-08-23).** Le remplacement de la
jokerisation automatique par un simple « à revalider » a été implémenté (`4f90785`) puis
**reverté à la demande du user**, qui a retiré son idée. Le comportement décrit dans le
tableau ci-dessus est donc bien celui en vigueur. Le code de la tentative reste dans
l'historique git — il embarquait une contrepartie obligatoire, le réalignement des
horaires du créneau à la revalidation, sans laquelle la modification devenait invisible
...
~~**F-05 (échange de shifts) reste non semé**~~ — vrai jusqu'au **2026-09-03** : le jeu de
démo pose désormais **une demande en attente** (D-90), sur un jour encore à venir de la
semaine courante, seule fenêtre où la route accepte un échange (futur ET publié).
...
| D-18 | F-05 — Échange de shifts : collection `shift_swaps`, 7 routes backend, modale patron (✓/✗ + raison), modale staff (4 semaines glissantes, cross-établissement). *Désactivée de mai 2026 au 2026-09-03, réactivée en D-90* | — |
...
| D-90 | **F-05 réactivée** (`server.js`, `public/planning.{js,html}`, `public/script.js`, `public/index.html`, `eslint.config.js`, `scripts/seed-demo.js`, `public/lib/nouveautes.js`, `tests/shift-swaps.test.js`) — demandé pour **présenter l'échange de shifts au client** : actif sur `dev` et sur la démo, **pas** en prod client. Le backlog annonçait « retirer des commentaires » ; c'était faux sur trois points, et chacun aurait été visible en démo. **(1) B2-b n'existait pas en mai 2026** : `shifts-for-swap` listait les shifts des collègues **sans filtre de publication**, donc le brouillon du patron devenait négociable — et `POST /api/shift-swaps` l'acceptait. Garde `publishedShiftFilter()` ajoutée **au point d'écriture** (403) autant que dans la liste ; la liste alimente l'écran, elle ne protège pas la route. **(2) Le point d'entrée staff était mort** : depuis, taper une carte du jour ouvre le mini planning (`day-card--tappable`, écoute déléguée sur `document`) — le clic sur l'en-tête que F-05 posait se serait battu avec lui. La carte ne porte plus que le **badge d'état** ; l'action (« ⇄ Proposer un échange » / « Annuler ma demande ») vit dans le **pied de la feuille du jour**, qui se referme avant d'ouvrir la modale (`z-index` 9998 vs 9999). **(3) Zéro test** malgré « code écrit et testé » au backlog : `tests/shift-swaps.test.js`, **17 tests** (propriété du shift, Joker, passé, brouillon, doublon, périmètre directeur, permutation effective des porteurs **et des couleurs**, 409/410, annulation). **11e lacune de `fake-db`** trouvée en les écrivant : `cursor.project()` n'existait pas — les deux requêtes qui s'en servent levaient un TypeError, la feature était **intestable**, pas seulement non testée. Le bloc `globals` F-05 d'`eslint.config.js` (qui déclarait `openSwapModal`/`cancelMySwap` en globales fantômes) est supprimé : elles sont maintenant vraiment définies. Jeu de démo : une demande en attente + ses notifs patron/directeur. 2 entrées « Du neuf » (staff, patron) | — |
| **Sprint sept. 2026 — clôture OTP → heures réelles** ||
| D-91 | **Clôture de service OTP (début + fin) + sync `real_*` + audit** (`server.js`, `public/pointage.{html,js}`, `public/planning.{html,js}`, `tests/cloture-otp.test.js`, `scripts/smoke-cloture.js`) — **Décision** : le code 4 chiffres (TTL 15 min, usage unique, collection `codes_cloture`) pointe **début puis fin** ; les champs `*_code` restent la **preuve immutable** ; les `*_finale` sont ajustables ; dès que la **paire** de retenues est complète, `syncRealHoursFromCloture` écrit `real_start`/`real_end` (quart d'heure) + snapshot salarial au premier sync, et append `time_validations` (`resultat: 'sync_real'`). Pas de sync sur début seul (D-71). **UI manager** : panneau dans Pointage (plus de bouton séparé dashboard) — **Clôture du jour** (nav ±1 j, date FR) ; ancien formulaire cartes masqué pour patron/directeur ; **extra non planifié** et CA conservés. **Staff** : CTA sur carte du jour (`planning.js`). **Extra** : `POST /api/shifts/extra` pose aussi les champs clôture manuels. Tests + smoke. Affiné en D-92. | — |
| D-92 | **Pointage / clôture — polish post-D-91** (`pointage.{html,js}`, `server.js`, `tests/cloture-otp.test.js`) — (1) saisie manuelle / ajuster : **modale** champs `type=time` (plus de `prompt()`), style sheet + quart d'heure ; (2) **containement mobile** début/fin (modale + « Ajouter un service ») — empilement ≤900 px, `overflow` / `min-width:0` / chrome `type=time` ; (3) **après `patron_valide`** : le **responsable** ne peut plus ajuster (API 403 + bouton masqué) ; **patron / directeur** gardent la main. | — |
| D-93 | **Journal clôture jour** (`server.js`, `public/pointage.{html,js}`, `tests/pointage-verif.test.js`) — **Clôture du jour** : onglets Shifts / Journal + recherche. **Observateur** : manuel / ajuster / valider-récap / CA / extra OK ; **pas le code OTP** (`denyObservateurEdit` sur `GET .../code-cloture`). | — |
...
| D-101 | **Créneaux prédéfinis N+1** — `slot_offer` + ouverture masse, visibilité staff dès « Proposer les créneaux » (même non publié, même sans shift personnel), **affichage staff en grille semaine cliquable** (comme le planning patron). Flag `predefined_slots`. Couleur Joker au choix. Doc : `docs/design-creneaux-predefinis.md`. | ✅ |
...
| D-105 | **Arbitrage A — Formule 3 inclut la saisie manuelle des heures** — `canWriteRealHours` = `time_tracking` OU `performance` ; `PATCH /api/shifts/:id/pointage` via `requireFeatureAny` ; UI patron (modale desktop + feuille mobile). OTP / tablette / comptes établissement restent `time_tracking` seul. Doc `feature-flags.md`. | ✅ |
| D-106 | **Refonte Pointage — dates / modes / suppression patron** — (1) Date unique : `setActiveDate` cale `today` + `_cloturesDay` ; flèches ←→ appellent `navigatePointageDay` → même jour pour sélecteur, CA, code OTP, clôture et cartes. (2) Modes : une seule liste par compte — clôture du jour pour les managers, saisie directe (`#legacy-pointage-block`) pour la tablette (les deux ensemble faisaient doublon, retiré le 2026-09-18) ; refresh via `refreshPointageDay`. (3) Patron/directeur peuvent `DELETE …/pointage` même si `real_*` déjà saisis (tablette reste en 409) ; bouton Supprimer sur cartes + lignes clôture. Tests `pointage-delete.test.js`. | ✅ |
| D-107 | **Pack démo prospect** — `.env.demo.example` avec `FEATURE_TIME_TRACKING` + `PERFORMANCE` + `SHIFT_SWAPS` ; guide `docs/guide-demo-prospect.md` + page `/demo-guide.html` ; seed-demo affiche les flags et pointe le guide ; dossier-projet / feature-flags mis à jour. iCal volontairement off. | ✅ |
| D-110 | **Validation hebdo : périmètre du responsable de soirée** — `validationEstabFilter` passait un compte staff par `assigned_establishments` (champ directeur) → périmètre vide : page Validation sans personne, `sign` → 404 « Aucun shift à signer », donc récap patron bloqué en 409 (« aucun des deux ne fonctionne », 2026-09-18). Modèle (patron, 2026-09-18) : **n'importe quel staff au rôle responsable signe n'importe qui**, pas seulement l'équipe des soirées où il était désigné → périmètre staff = tout (`canAccessWeekValidation` filtre déjà sur le rôle). Bouton « Validation » du planning piloté par `can_validate` de `/api/me/week-sign-code` (rôle, quelle que soit la semaine). Page **regroupée par établissement** (en-tête = affaire + compteur signés), une carte par affaire pour un staff multi-affaires (heures de l'affaire, mention « aussi : … »), signature unique ; `/api/validation/week` renvoie `establishments[{id,name}]`. Tests dans `week-validation.test.js`. | ✅ |
| D-109 | **Ancien pointage présent dans la clôture** — la tablette, la saisie directe et la modale « heures réelles » du planning n'écrivaient que `real_start/real_end` ; la clôture du jour, le récap (`valider-recap`) et la file à valider ne lisent que `heure_validee_finale` → ces shifts restaient « Non commencés ». `clotureFieldsFromRealHours` (`lib/utils.js`) fait le miroir dans `PATCH /api/shifts/:id/pointage` (retenus + `*_code` si absents, source `manuelle`, journal `saisie_directe` / `DIRECT`) ; effacer les heures rouvre la clôture, refusé (409) si récap validé. Rattrapage de l'existant : `scripts/backfill-cloture-from-real.js` (simulation par défaut, `--apply`). Tests `pointage-saisie-directe-cloture.test.js`. | ✅ |
| D-108 | **Démo à l'échelle d'un groupe (8 établissements)** — `seed-demo.js` : 2 établissements du récit (Le Zinc, La Rotonde) + 6 satellites générés (4 bars / 4 restaurants dont un midi seul), 78 personnes, ~2750 shifts. Services DÉRIVÉS des horaires (`derivedService`), équipes générées par gabarit (`TEAM`, 3–4 responsables par établissement sinon ils raflent tous les créneaux `resp`), objectif de coefficient par affaire posé sur la définition de l'établissement, Le Perchoir volontairement sous son objectif pour que la vue consolidée serve à quelque chose. Corrige au passage le numéro de téléphone (débordait à 11 chiffres au-delà de 26 personnes) et écarte la directrice du tirage de l'échange. Guide + `/demo-guide.html` alignés. Motif : un prospect qui exploite 8 adresses ne se reconnaît pas dans un groupe de 2. | ✅ |
...
## Backlog Pointage / Clôture OTP (sept. 2026)

Registre **local** au domaine Pointage (page `pointage.html` + CTA staff + sync `real_*`).
Les IDs `PT-xx` vivent aussi dans `docs/ux-design.md` §2.5.

### Livré

| ID | Description | Statut |
|---|---|---|
| E-03 / D-15 | Onglet Pointage responsable / directeur | ✅ |
| PT-01→PT-08 / D-21 / D-67 | UX cartes, édition, footer, écarts, bandeau soirée, quart d'heure, préremplissage | ✅ |
| D-54 | Suppression shift non pointé depuis Pointage | ✅ |
| PT-09 / D-91 | Clôture OTP début+fin, sync `real_*`, UI manager, CTA staff, extra + CA | ✅ |
| PT-10 / D-91→D-92 | Ajuster après récap : **oui** pour patron/directeur ; **non** pour responsable | ✅ |
| D-92 | Modale heures (vs `prompt`), champs dans le cadre mobile/tablette | ✅ |
| D-93 / PT-16 | Journal dans Clôture du jour (jour + recherche nom) ; pastille/file Vérification retirées | ✅ |
| D-94 | Observateur hors Dispos/Échanges ; clic shift no-op ; joker passé retiré ; recherche échange | ✅ |
| — | Historique litiges (`time_validations`) bouton + API | ✅ |
| — | Responsable = soirée active seulement (`getActivePointageDateStr`) | ✅ |

### Ouvert — à prioriser

| ID | Description | Prio | Notes |
|---|---|---|---|
| PT-11 | **Unifier tablette établissement** — le compte `etablissement` garde encore l'ancienne grille cartes ; décider : basculer sur l'UI clôture, ou garder volontairement deux modes | P2 | Hors scope D-91 ; coexiste aujourd'hui |
| ~~PT-12~~ | ~~**Note client** — qui ajuste après récap + panel vérif~~ | P2 | ✅ Done |
| PT-13 | **Écran litiges enrichi** — vue jour / filtre / export au-delà du journal panel | P3 | Journal D-93 couvre l'essentiel |
| PT-14 | **Tests front Pointage** — zéro couverture UI (`pointage.js`) ; smoke serveur seulement | P3 | T-03 global |
| PT-15 | Extraire `matchesWordPrefix` partagé (D-88) hors duplication `pointage.js` / `script.js` | P3 | Dette légère |

### Invariants (ne pas casser)

- Preuve = `*_code` (immutables) ; retenues = `*_finale` ; `real_*` seulement si **paire** complète.
- `time_validations` **append-only**.
- Route `GET /api/shifts/:id/time-validations` **avant** `/:establishmentId/:date`.
- Smoke : `npm run smoke:cloture` · tests : `tests/cloture-otp.test.js`.
...
- **server.js** : monolithique (**6281 lignes, 118 routes** au 2026-08-14 — il a pris ~2000 lignes et 17 routes depuis juin). Helpers purs dans `lib/utils.js` (testés). Split en routers = chantier futur (#10 backlog, voir R-04). ⚠️ ~~Deux blocs `/* F-05 DÉSACTIVÉ */`~~ **supprimés le 2026-09-03 (D-90)** : les routes d'échange sont actives, il n'y a plus de bloc commenté où une route pourrait se perdre (le piège de D-47 : invisible à Express, 404 silencieux). Reste la leçon générale — **ne jamais commenter un bloc de routes**, un flag de configuration se lit, un `/* */` de 240 lignes non.
...
- **Tests** : `npm test` (zéro dépendance, `node --test`) — **366 tests, 16 fichiers** au 2026-08-14 (cf. « État actuel des tests »). Les tests d'intégration requièrent `server.js` (qui exporte `app` et ne démarre/se connecte que si `require.main === module`), démarrent l'app sur un port éphémère et injectent un faux Mongo via `tests/helpers/fake-db.js` + `app.locals.setTestDb` ; la session vient de l'en-tête `x-test-user`. **Toute la config d'env est dans `tests/helpers/harness.js`** — ne jamais la redupliquer dans un fichier de test. ⚠️ **`fake-db` est un sous-ensemble de l'API Mongo** : **11 lacunes** ont déjà été trouvées à l'usage (`deleteOne`, `distinct`, `$and`/`$or`/`$exists`, `updateMany`, copies défensives de `find`/`findOne`, `$addToSet` + `$pull` par critère, `_id` posé par l'upsert de `bulkWrite`, `sort()` no-op, `cursor.project()` absent). Un test vert ne prouve pas que la vraie requête tourne ; le seul niveau qui exerce Mongo est `npm run smoke`. ⚠️ le mode répertoire `node --test tests/` **n'est pas fiable** (échoue selon la version Node) : lister les fichiers explicitement dans `package.json`. Ajouter un test quand on extrait un helper pur, change une règle de date/heure, ou fixe un bug qui pourrait régresser.
...
- **Clôture OTP (D-91 / D-92 / D-93)** : preuve = `*_code` ; retenues = `*_finale` ; sync `real_*` si paire complète. Après `patron_valide` : patron/directeur/**observateur** peuvent ajuster ; **responsable** non. **Observateur** : pas de code OTP. Journal du jour dans **Clôture du jour**. Tests : `cloture-otp` + `pointage-verif`. Smoke : `npm run smoke:cloture`.
- **Observateur (D-86 / D-93 / D-94)** : admin OK ; **pas** construction planning ; **pas** UI/API Dispos ni Échanges à valider ; clic shift no-op ; OTP interdit. Front = `body.observateur` ; serveur = `denyObservateurEdit`.
- **Simulation Performance (D-95)** : onglet distinct ; calcul éphémère ; hybride réel pointé + planifié ; jokers via manuel ou mean/median (`deriveStaffHourlyStat` / `buildPerformanceSimulation`). Ne jamais écrire `daily_revenue` depuis la simu. **Personne sans taux (D-104)** : côté estimé seulement, elle est costée au taux de son groupe comme un Joker ; côté réel un taux absent n'est JAMAIS estimé — la colonne « réel » doit rester réelle.
- **Joker ouvert passé (D-94)** : dès `date < today` (cutoff pointage), soft-close `joker_open` + hors liste `joker-ouverts` / candidature refusée.
