...
| D-93 | **Journal clôture jour** (`server.js`, `public/pointage.{html,js}`, `tests/pointage-verif.test.js`) — sur **`pointage.html`**, dans **Clôture du jour** : onglets **Shifts** / **Journal** (jour sélectionné uniquement) + **recherche par nom** ; pastille/section **Vérification** retirée (doublon). Routes `GET /api/pointage/verif/{count,pending,journal}` conservées (journal filtré `from=to=jour` + établissement). `valider-recap` journalisé ; observateur aligné sur la clôture. | — |
...
- **Clôture OTP (D-91 / D-92 / D-93)** : preuve = `*_code` ; retenues = `*_finale` ; sync `real_*` si paire complète. Après `patron_valide` : patron/directeur/**observateur** peuvent ajuster ; **responsable** non. Journal du jour dans **Clôture du jour** (`GET /api/pointage/verif/journal`). Tests : `cloture-otp` + `pointage-verif`. Smoke : `npm run smoke:cloture`.
