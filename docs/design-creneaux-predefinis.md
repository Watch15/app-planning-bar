...
*Cadré 2026-09-10. Flag : `predefined_slots` (profil Castaniu, **on par défaut** ; `FEATURE_PREDEFINED_SLOTS=false` pour forcer off).*
...
5. **Visibilité staff** : **uniquement** après « Proposer les créneaux ». Les `slot_offer` sont alors visibles même si la semaine n’est pas publiée et même si le staff n’a aucun shift. Affichage au choix staff : **Liste** (lignes jour + boutons) ou **Tableau de bord** (grille horaires) — bascule mémorisée ; défaut liste sur téléphone, tableau sur ordi. Un tap / bouton candidature. Un staff **déjà en shift sur les mêmes horaires** ne peut pas postuler (barre « Déjà en shift », 409 côté API). Les Jokers ponctuels (hors `slot_offer`) restent en liste « Créneaux disponibles ». Sans le clic patron, rien n’est annoncé. Le staff **peut se retirer** tant que le Joker est ouvert ; le patron peut aussi retirer une candidature. Le **nom de groupe n’est pas affiché** côté staff (filtre serveur uniquement). **À l’assignation** d’un staff sur un créneau, il est **retiré automatiquement** de toutes ses autres candidatures du même jour dont les horaires chevauchent (enchaînement pile : pas de retrait).
