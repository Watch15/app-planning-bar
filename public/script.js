...
async function init() {
    ...
    loadSwapsBadge();
    loadNotifBadge();
    _notifPollTimer = setInterval(() => { loadNotifBadge(); loadSwapsBadge(); loadPointageBadge(); }, 30000);
    startAutoRefresh();
    initNotifListeners();
    loadDispoControl();
    initStaffSearch();
...
// ── Vérification Pointage (panel patron) ─────────────────────────────────────

let _pvWeekStart = null;
let _pvJournalWeek = null;
let _pvAjustCtx = null;
let _pvBound = false;
...
function _pvSourceBadge(label) {
    if (!label) return '';
    const isOtp = label === 'Code OTP';
    const isMan = label === 'Saisie manuelle';
    const bg = isOtp ? '#e0f2fe' : isMan ? '#fef3c7' : '#f1f5f9';
    const fg = isOtp ? '#0369a1' : isMan ? '#92400e' : '#475569';
    return '<span style="display:inline-block;font-size:10px;font-weight:700;padding:2px 6px;border-radius:6px;background:' + bg + ';color:' + fg + ';margin-left:4px">' + escapeHtml(label) + '</span>';
}

function _pvMondayOf(dateStr) {
    const d = new Date(dateStr + 'T12:00:00');
    return window.Week ? Week.toDateStr(Week.weekStart(d)) : dateStr;
}
...
function switchPointageVerifTab(tab) {
    ...
    const active = '2px solid var(--accent)';
    const idle = '2px solid transparent';
    if (tab === 'journal') {
        if (queue) queue.style.display = 'none';
        if (journal) journal.style.display = '';
        if (btnQ) { btnQ.style.borderBottom = idle; btnQ.style.color = 'var(--text-secondary)'; btnQ.style.fontWeight = '500'; }
        if (btnJ) { btnJ.style.borderBottom = active; btnJ.style.color = 'var(--accent)'; btnJ.style.fontWeight = '600'; }
        loadPointageVerifJournal();
    } else {
        if (queue) queue.style.display = '';
        if (journal) journal.style.display = 'none';
        if (btnJ) { btnJ.style.borderBottom = idle; btnJ.style.color = 'var(--text-secondary)'; btnJ.style.fontWeight = '500'; }
        if (btnQ) { btnQ.style.borderBottom = active; btnQ.style.color = 'var(--accent)'; btnQ.style.fontWeight = '600'; }
        loadPointageVerifQueue();
    }
}

async function openPointageVerifPanel() {
    const modal = document.getElementById('pointage-verif-modal');
    if (!modal) return;
    if (!_pvWeekStart) {
        try {
            const res = await fetch('/api/pointage/verif/count', { credentials: 'include' });
            if (res.ok) {
                const data = await res.json();
                _pvWeekStart = data.week_start || _pvMondayOf(data.active_date || Week.toDateStr(new Date()));
            }
        } catch { /* */ }
        if (!_pvWeekStart) _pvWeekStart = _pvMondayOf(Week.toDateStr(new Date()));
    }
    _pvJournalWeek = _pvWeekStart;
    modal.style.display = 'flex';
    switchPointageVerifTab('queue');
}

function closePointageVerifPanel() {
    const modal = document.getElementById('pointage-verif-modal');
...
async function loadPointageVerifQueue() {
    ...
    list.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-muted);font-size:13px">Chargement…</div>';
    try {
        ...
        if (!items.length) {
            list.innerHTML = '<div style="padding:28px;text-align:center;color:var(--text-muted);font-size:13px">Rien à vérifier pour cette semaine.</div>';
            return;
        }
        let html = '';
        const renderSection = (title, rows) => {
            if (!rows.length) return;
            html += '<div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;margin:12px 4px 8px">' + escapeHtml(title) + ' (' + rows.length + ')</div>';
            rows.forEach(s => {
                html += '<div class="pv-row" data-id="' + escapeHtml(String(s._id)) + '" style="border:1px solid var(--light-border);border-radius:10px;padding:10px 12px;margin-bottom:8px;background:var(--light-bg)">';
                html += '<div style="display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap">';
                html += '<div><div style="font-weight:700;font-size:14px;color:var(--text-primary)">' + escapeHtml(s.staff_name || '—') + '</div>';
                html += '<div style="font-size:12px;color:var(--text-muted);margin-top:2px">' + escapeHtml(s.establishment_name || '') + ' · ' + escapeHtml(s.date || '') + '</div></div>';
                html += '<div class="pv-row-actions" style="display:flex;gap:6px;flex-wrap:wrap"></div></div>';
                html += '<div style="font-size:12px;margin-top:8px;color:var(--text-secondary)">Planifié ' + escapeHtml(_pvFmtH(s.start_time)) + '–' + escapeHtml(_pvFmtH(s.end_time)) + '</div>';
                html += '<div style="font-size:12px;margin-top:4px">Début : <strong>' + escapeHtml(s.debut_valide_finale || '—') + '</strong>' + _pvSourceBadge(s.debut_source_label) + '</div>';
                html += '<div style="font-size:12px;margin-top:2px">Fin : <strong>' + escapeHtml(s.heure_validee_finale || '—') + '</strong>' + _pvSourceBadge(s.cloture_source_label) + '</div>';
                html += '</div>';
            });
        };
        renderSection('Non clôturés (soirée active)', nonClot);
        renderSection('Clôturés à valider', aVal);
        list.innerHTML = html;
        list.querySelectorAll('.pv-row').forEach(row => {
            const id = row.getAttribute('data-id');
            const item = items.find(x => String(x._id) === id);
            if (!item) return;
            const actions = row.querySelector('.pv-row-actions');
            if (item.statut === 'non_cloture') {
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.textContent = item.debut_valide_code ? 'Fin manuelle' : 'Clôturer';
                btn.style.cssText = 'padding:6px 10px;border-radius:8px;border:1.5px solid var(--light-border);background:#fff;font-size:12px;font-weight:600;cursor:pointer';
                btn.addEventListener('click', () => openPvManuel(item));
                ...
                btn.style.cssText = 'padding:6px 10px;border-radius:8px;border:1.5px solid var(--light-border);background:#fff;font-size:12px;font-weight:600;cursor:pointer';
                btn.addEventListener('click', () => openPvAjust(item));
                actions.appendChild(btn);
            }
        });
    } catch (e) {
        list.innerHTML = '<div style="padding:16px;color:#b91c1c;font-size:13px">' + escapeHtml(e.message) + '</div>';
    }
}
...
async function loadPointageVerifJournal() {
    ...
    try {
        ...
        if (!res.ok) throw new Error(data.error || 'Erreur');
        if (!data.length) {
            list.innerHTML = '<div style="padding:28px;text-align:center;color:var(--text-muted);font-size:13px">Aucun mouvement sur cette semaine.</div>';
            return;
        }
        list.innerHTML = data.map(v => {
            const acteur = v.acteur_name || v.acteur_id || '—';
            const role = v.acteur_role ? ' (' + v.acteur_role + ')' : '';
            return '<div style="border:1px solid var(--light-border);border-radius:10px;padding:10px 12px;margin-bottom:8px;background:var(--light-card)">'
                + '<div style="display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap">'
                + '<strong style="font-size:13px">' + escapeHtml(v.staff_name || '—') + '</strong>'
                + '<span style="font-size:11px;color:var(--text-muted)">' + escapeHtml(_pvFmtParts(v.heure_saisie)) + '</span>'
                + '</div>'
                + '<div style="font-size:12px;margin-top:4px;color:var(--text-secondary)">'
                + escapeHtml(v.establishment_name || '') + (v.shift_date ? ' · ' + escapeHtml(v.shift_date) : '')
                + '</div>'
                + '<div style="font-size:12px;margin-top:6px">Par <strong>' + escapeHtml(acteur) + '</strong>' + escapeHtml(role)
                + _pvSourceBadge(v.source_label)
                + '</div>'
                + '<div style="font-size:12px;margin-top:4px;color:var(--text-muted)">'
                + escapeHtml([v.action, v.phase, v.resultat].filter(Boolean).join(' · '))
                + (v.debut_retenue || v.fin_retenue ? ' · ' + escapeHtml((v.debut_retenue || '—') + '–' + (v.fin_retenue || '—')) : '')
                + '</div>'
                + (v.motif ? '<div style="font-size:12px;font-style:italic;margin-top:4px;color:var(--text-secondary)">' + escapeHtml(v.motif) + '</div>' : '')
                + '</div>';
        }).join('');
    } catch (e) {
        list.innerHTML = '<div style="padding:16px;color:#b91c1c;font-size:13px">' + escapeHtml(e.message) + '</div>';
...
function openPvAjust(item) {
    _pvAjustCtx = { mode: 'ajust', item };
    const modal = document.getElementById('pv-ajust-modal');
    document.getElementById('pv-ajust-title').textContent = 'Ajuster — ' + (item.staff_name || '');
    document.getElementById('pv-ajust-hint').textContent = 'Les heures d\'origine (OTP/manuel) restent inchangées.';
    document.getElementById('pv-ajust-debut-wrap').style.display = '';
    document.getElementById('pv-ajust-fin-wrap').style.display = '';
    document.getElementById('pv-ajust-motif-wrap').style.display = '';
    document.getElementById('pv-ajust-debut').value = _pvToTimeInput(item.debut_valide_finale);
    document.getElementById('pv-ajust-fin').value = _pvToTimeInput(item.heure_validee_finale);
    document.getElementById('pv-ajust-motif').value = item.motif_modification || '';
    if (modal) modal.style.display = 'flex';
}

function openPvManuel(item) {
    const hasDebut = !!item.debut_valide_code;
    _pvAjustCtx = { mode: hasDebut ? 'fin' : 'both', item };
    const modal = document.getElementById('pv-ajust-modal');
    document.getElementById('pv-ajust-title').textContent = (hasDebut ? 'Fin manuelle' : 'Clôture manuelle') + ' — ' + (item.staff_name || '');
    document.getElementById('pv-ajust-hint').textContent = 'Saisie manuelle (enregistrée comme telle dans le journal).';
    document.getElementById('pv-ajust-debut-wrap').style.display = hasDebut ? 'none' : '';
    document.getElementById('pv-ajust-fin-wrap').style.display = '';
    document.getElementById('pv-ajust-motif-wrap').style.display = 'none';
    document.getElementById('pv-ajust-debut').value = _pvToTimeInput(item.debut_valide_finale) || _pvToTimeInput(item.start_time);
    document.getElementById('pv-ajust-fin').value = _pvToTimeInput(item.end_time);
    if (modal) modal.style.display = 'flex';
}

function closePvAjust() {
    const modal = document.getElementById('pv-ajust-modal');
    if (modal) modal.style.display = 'none';
    _pvAjustCtx = null;
}

async function submitPvAjust() {
    if (!_pvAjustCtx) return;
    const { mode, item } = _pvAjustCtx;
    const debut = (document.getElementById('pv-ajust-debut').value || '').trim();
    const fin = (document.getElementById('pv-ajust-fin').value || '').trim();
    const motif = (document.getElementById('pv-ajust-motif').value || '').trim();
    const btn = document.getElementById('pv-ajust-submit');
    ...
    try {
        if (mode === 'ajust') {
            ...
            if (!body.debut_valide_finale && !body.heure_validee_finale) throw new Error('Indique au moins une heure');
            const res = await fetch('/api/shifts/' + item._id + '/ajuster-heure', {
                method: 'PATCH', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Erreur');
            showToast('Heures ajustées');
        } else {
            const body = { phase: 'fin' };
            if (fin) body.heure = fin;
            if (mode === 'both') {
                if (!debut) throw new Error('Heure de début requise');
                body.heure_debut = debut;
            }
            const res = await fetch('/api/shifts/' + item._id + '/cloturer-manuel', {
                method: 'POST', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(body),
            });
...
async function validatePointageRecapFromPanel() {
    if (!_pvWeekStart) return;
    const estabs = (typeof allEstablishments !== 'undefined' && allEstablishments)
        ? allEstablishments
        : [];
    // Patron/observateur : tous les bars chargés ; directeur : déjà filtrés côté loadEstablishments
    const ids = estabs.map(e => e.id).filter(Boolean);
    if (!ids.length) {
        showToast('Aucun établissement', true);
        return;
    }
    if (!confirm('Valider le récap de la semaine du ' + _pvWeekStart + ' pour ' + ids.length + ' établissement(s) ?')) return;
    let modified = 0;
    try {
        for (const id of ids) {
            const res = await fetch('/api/etablissements/' + encodeURIComponent(id) + '/valider-recap', {
                method: 'POST', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
        ...
        showToast(modified ? ('Récap validé (' + modified + ')') : 'Rien à valider');
        loadPointageBadge();
        loadPointageVerifQueue();
    } catch (e) {
        showToast(e.message, true);
...
function initPointageVerifPanel() {
    ...
    if (btn) btn.addEventListener('click', openPointageVerifPanel);
    document.getElementById('pointage-verif-close')?.addEventListener('click', closePointageVerifPanel);
    document.getElementById('pointage-verif-modal')?.addEventListener('click', e => {
        if (e.target.id === 'pointage-verif-modal') closePointageVerifPanel();
    });
    ...
    document.getElementById('pv-ajust-close')?.addEventListener('click', closePvAjust);
    document.getElementById('pv-ajust-submit')?.addEventListener('click', submitPvAjust);
    document.getElementById('pv-ajust-modal')?.addEventListener('click', e => {
        if (e.target.id === 'pv-ajust-modal') closePvAjust();
    });
}
...
async function silentRefresh() {
    ...
    try { loadPointageBadge(); } catch { /* silencieux */ }
