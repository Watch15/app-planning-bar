...
const FEATURES = {
    ...
    // Sous-module de `time_tracking` : le code OTP à 4 chiffres (début / fin de service).
    // Périmètre exact et distinction avec le code SEMAINE : docs/feature-flags.md.
    otp_closure: {
...
function enabled(featureKey, opts) {
    ...
    if ((def.requires || []).some(dep => !enabled(dep, opts))) return false;
