...
function normalizePhone(raw) {
    ...
    // 2. "+33(0)6..." / "+32(0)4..." / "+41(0)7..." → enlever le 0 national entre parenthèses
    p = p.replace(/\+(\d{1,3})\(0\)/g, '+$1');

    // 3. Supprimer espaces, tirets, points, parenthèses
    p = p.replace(/[\s\-.()]/g, '');

    // 4. "0032..." / "0041..." / "0033..." → "+32..." / "+41..." / "+33..."
    if (/^00/.test(p)) p = '+' + p.slice(2);

    // 5. Formats nationaux → E.164 (FR / BE / CH)
    //    FR mobile 06/07 (10 chiffres) → +33…  (historique Templyo, inchangé)
    //    BE mobile 045–049 (10 chiffres) → +32…  (les fixes FR en 04 ne servent
    //      pas aux SMS d'invitation ; le staff saisit un mobile)
    //    CH : le 07x local est ambigu avec le mobile FR → exiger +41 / 0041
    if (/^0[67]\d{8}$/.test(p)) {
        p = '+33' + p.slice(1);
    } else if (/^04[5-9]\d{7}$/.test(p)) {
        p = '+32' + p.slice(1);
    }

    // 6. Pas de "+" → ajouter (numéro déjà international sans préfixe)
    if (!/^\+/.test(p)) p = '+' + p;

    // 7. Trunk 0 oublié après indicatif FR/BE/CH : "+3306…" → "+336…",
    //    "+3204…" → "+324…", "+4107…" → "+417…"
    p = p.replace(/^\+(33|32|41)0(\d+)/, '+$1$2');
    ...
    // 9. Longueurs nationales attendues (NSN) pour les 3 pays supportés.
    //    Évite d'accepter un +32/+41 tronqué qui passerait l'E.164 générique.
    const nsn = {
        '33': /^\+33\d{9}$/,   // FR : 9 chiffres après +33
        '32': /^\+32\d{8,9}$/, // BE : 8–9 chiffres après +32
        '41': /^\+41\d{9}$/,   // CH : 9 chiffres après +41
    };
    for (const [cc, re] of Object.entries(nsn)) {
        if (p.startsWith('+' + cc) && !re.test(p)) {
            console.error('[normalizePhone] longueur invalide (' + cc + ') :', String(raw).slice(0, 40), '→', p);
            return null;
        }
    }
...
/** Clé de valo Simulation : groupe du joker, ou '_' si absent (legacy / sans groupe). */
function jokerGroupKey(shift) {
    if (shift && shift.joker_group != null && String(shift.joker_group).trim() !== '') {
        return String(shift.joker_group);
    }
    return '_';
}
...
function filterStaffForRateStats(staffList, opts) {
    const o = opts || {};
...
function deriveStaffHourlyStat(staffList, opts) {
    ...
    const list = filterStaffForRateStats(staffList, o);
...
/** Agrégats mean/median + comptages horaire/forfait pour le panel Performance (D-98). */
function summarizeStaffRateBucket(staffList) {
    const list = Array.isArray(staffList) ? staffList : [];
    let n_hourly = 0;
    let n_fixed = 0;
    list.forEach(s => {
        if (!s) return;
        if (s.fixed_rate != null && s.fixed_rate !== '' && !Number.isNaN(Number(s.fixed_rate))) n_fixed += 1;
        else if (s.hourly_rate != null && s.hourly_rate !== '' && !Number.isNaN(Number(s.hourly_rate))) n_hourly += 1;
    });
    const mean = deriveStaffHourlyStat(list, { stat: 'mean' });
    const median = deriveStaffHourlyStat(list, { stat: 'median' });
    return {
        mean: mean.rate,
        median: median.rate,
...
/**
 * Rapport taux staff pour Performance Réel.
 * @returns {{ groups: Array, overall: object }}
 */
function buildStaffRateStatsReport(staffList, opts) {
    const o = opts || {};
    const filtered = filterStaffForRateStats(staffList, {
        establishmentIds: o.establishmentIds,
        groupIds: o.groupIds,
    });
    const overall = summarizeStaffRateBucket(filtered);
    const groupKeys = new Set();
    if (Array.isArray(o.groupIds) && o.groupIds.length > 0) {
        o.groupIds.forEach(g => groupKeys.add(String(g)));
    } else {
        filtered.forEach(s => {
            (Array.isArray(s.groups) ? s.groups : []).forEach(g => {
                if (g != null && String(g).trim() !== '') groupKeys.add(String(g));
            });
        });
    }
    const groups = [...groupKeys].sort((a, b) => a.localeCompare(b, 'fr')).map(key => {
        const subset = filtered.filter(s => Array.isArray(s.groups) && s.groups.includes(key));
        return Object.assign({ key, label: key }, summarizeStaffRateBucket(subset));
    });
    return { groups, overall };
}

// Le groupe qui porte le taux d'une personne. Un shift Joker en désigne UN
// (`joker_group`) ; un staff en porte une LISTE — on prend le premier par ordre
// alphabétique, celui que sa fiche affiche déjà en tête, pour que deux simulations
// du même staff ne changent pas de taux d'une fois sur l'autre. Sans groupe : '_',
// exactement comme un Joker sans groupe.
function staffGroupKey(staffDoc) {
    const groups = staffDoc && Array.isArray(staffDoc.groups)
        ? staffDoc.groups.filter(g => g != null && String(g).trim() !== '').map(String)
        : [];
    if (groups.length === 0) return '_';
    return groups.slice().sort((a, b) => a.localeCompare(b, 'fr'))[0];
}

// Le taux de repli d'un groupe, tel que la Simulation l'a résolu : forfait puis
// horaire, par groupe puis global. Commun aux Jokers et aux personnes sans taux —
// c'est la même question (combien coûte ce créneau que personne ne tarifie ?), donc
// la même réponse. Retourne null si rien n'est résolu pour ce groupe.
function groupFallbackRate(g, o, hours) {
    const fixedByG = o.jokerFixedByGroup && o.jokerFixedByGroup[g];
    const hourlyByG = o.jokerHourlyByGroup && o.jokerHourlyByGroup[g];
    if (fixedByG != null && !Number.isNaN(Number(fixedByG))) {
        return { wage: Number(fixedByG), rate: Number(fixedByG), is_fixed: true };
    }
    if (hourlyByG != null && !Number.isNaN(Number(hourlyByG))) {
        return { wage: hours * Number(hourlyByG), rate: Number(hourlyByG), is_fixed: false };
    }
    if (o.jokerFixed != null && !Number.isNaN(Number(o.jokerFixed))) {
        return { wage: Number(o.jokerFixed), rate: Number(o.jokerFixed), is_fixed: true };
    }
    if (o.jokerHourly != null && !Number.isNaN(Number(o.jokerHourly))) {
        return { wage: hours * Number(o.jokerHourly), rate: Number(o.jokerHourly), is_fixed: false };
    }
    return null;
}
...
function wageLineForShift(shift, staffDoc, opts) {
    ...
    let rate_fallback = false, fallback_group = null;
    ...
    if (completed) {
        ...
        if (joker) {
            const g = jokerGroupKey(shift);
            const r = groupFallbackRate(g, o, hours);
            if (r) { wage = r.wage; rate = r.rate; is_fixed = r.is_fixed; }
            else missing_rate = true;
        } else if (staffDoc && staffDoc.fixed_rate != null) {
            wage = staffDoc.fixed_rate;
            rate = staffDoc.fixed_rate;
            is_fixed = true;
        } else if (staffDoc && staffDoc.hourly_rate != null) {
            wage = hours * staffDoc.hourly_rate;
            rate = staffDoc.hourly_rate;
        } else {
            // Personne sans taux — ou fiche introuvable — sur un créneau ESTIMÉ : même
            // traitement qu'un Joker, le taux de son groupe résolu par le même réglage.
            // Avant, la ligne valait 0 € en silence : la masse était sous-comptée et le
            // coefficient faux sans que rien ne le dise.
            // ⚠️ Uniquement du côté estimé. Côté réel (`completed`), un taux absent reste
            // `missing_rate` : y appliquer une estimation inventerait une dépense jamais
            // faite et la colonne « réel » cesserait d'être réelle.
            const g = staffGroupKey(staffDoc);
            const r = groupFallbackRate(g, o, hours);
            if (r) {
                wage = r.wage;
                rate = r.rate;
    ...
    return {
        ...
        source,
        is_joker:     joker,
        joker_group:  joker ? (shift.joker_group || null) : null,
        staff_name,
    ...
    const days = [...dates].sort().map(date => {
        ...
        (byDate[date] || []).forEach(s => {
            ...
            const line = wageLineForShift(s, staffDoc, {
                chargeMult, jokerHourly, jokerFixed, jokerHourlyByGroup, jokerFixedByGroup,
            });
            if (!line) return;
            lines.push(line);
            if (line.source === 'real') {
...
/** Float heures (peut dépasser 24 : 26.5 = 02:30 le lendemain) → « HH:MM ». */
function hourFloatToHHMM(h) {
    const n = Number(h);
...
/**
 * Sens inverse : l'ancien pointage (real_start / real_end — tablette, saisie directe,
 * modale « heures réelles » du planning) doit exister dans la clôture (début/fin
 * retenus, source, journal), sinon le shift reste « Non commencé » et n'entre jamais
 * dans le récap ni la file à valider. Retourne les champs $set / $unset à appliquer
 * en plus de real_* ; `hours` = { real_start, real_end } tels que voulus après écriture.
 * Un `*_code` déjà posé (OTP ou manuel) n'est pas réécrit : seul le retenu bouge, comme
 * un ajustement.
 */
function clotureFieldsFromRealHours(shift, hours) {
    const set = {};
    const unset = {};
    if (hours.real_start != null) {
        const hhmm = hourFloatToHHMM(hours.real_start);
        set.debut_valide_finale = hhmm;
        if (!shift.debut_valide_code) { set.debut_valide_code = hhmm; set.debut_source = 'manuelle'; }
    } else {
        for (const k of ['debut_valide_code', 'debut_valide_finale', 'debut_source']) unset[k] = '';
    }
    if (hours.real_end != null) {
        const hhmm = hourFloatToHHMM(hours.real_end);
        set.heure_validee_finale = hhmm;
        if (!shift.heure_validee_code) { set.heure_validee_code = hhmm; set.cloture_source = 'manuelle'; }
    } else {
        for (const k of ['heure_validee_code', 'heure_validee_finale', 'cloture_source']) unset[k] = '';
    }
    return { set, unset };
}
...
module.exports = {
    ...
    filterStaffForRateStats,
    summarizeStaffRateBucket,
