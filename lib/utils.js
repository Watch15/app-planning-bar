...
// Informatif (le directeur reste NON planifiable, cf. E-19). Dispo effective d'une
// date = override du jour s'il existe, sinon la case du modèle hebdo récurrent
// (jour de semaine lundi=0 … dimanche=6), sinon « non renseigné ».

// Jour de semaine lundi-first (0=lun … 6=dim) d'une chaîne 'YYYY-MM-DD'. Midi local
// pour éviter tout glissement de fuseau.
function mondayFirstDow(dateStr) {
    return (new Date(dateStr + 'T12:00:00').getDay() + 6) % 7;
}

// `template` : doc { days: { 0..6: { available, start_time, end_time } } } ou null.
// `override` : doc jour { available, start_time, end_time } ou null/undefined.
function resolveManagerAvailability(template, override, dateStr) {
    const norm = (src, source) => ({
        date: dateStr,
        available:  !!src.available,
        start_time: src.available ? (src.start_time ?? null) : null,
        end_time:   src.available ? (src.end_time   ?? null) : null,
        source,
    });
    if (override) return norm(override, 'override');
    const cell = template && template.days ? template.days[mondayFirstDow(dateStr)] : null;
    if (cell) return norm(cell, 'template');
    return { date: dateStr, available: false, start_time: null, end_time: null, source: 'none' };
}
...
// (dispo manuelle → `takenDates`), renvoie { date, type, start_time, end_time } à
// insérer. `template.days` est keyé lundi=0 … dimanche=6 ; `weekStart` doit être un
// lundi ('YYYY-MM-DD'). Midi local → insensible au DST.
