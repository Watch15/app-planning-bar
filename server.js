...
// F-13 — « fait partie de l'équipe AUJOURD'HUI ». Un staff archivé sort de la vie
// courante (barre du planning, rappels, dispos, KPI, nouveaux shifts) sans que rien
// de son passé ne bouge : ses shifts, son pointage, son récap et sa masse salariale
// restent tels quels. C'est la différence avec `DELETE /api/staff/:id`, qui détruit
// l'historique de paie — et qui était jusqu'ici la SEULE sortie offerte au patron.
//
// Un seul objet, étalé dans chacune des requêtes concernées, plutôt que six copies
// littérales de `archived: { $ne: true }` : R-06 traîne déjà quatre copies d'une
// même règle, et c'est toujours la cinquième qu'on oublie de mettre à jour.
...
// F-14 — LA porte de la planification. F-13 n'avait posé le refus que dans
// `POST /api/shifts` : cinq autres chemins écrivaient un `staff_id` dans `shifts` sans
...
async function resolveStaffForPlanning({ staff_id, staff_name, is_joker } = {}) {
    ...
    if (staff_id && isValidObjectId(staff_id)) {
        prof = await db.collection('staff').findOne({ _id: new ObjectId(staff_id) }, projection);
    } else if (staff_name) {
        prof = await db.collection('staff').findOne({
            name: { $regex: '^' + String(staff_name).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$', $options: 'i' },
        }, projection);
    }

    if (prof && prof.archived === true) {
        return { denied: { status: 409,
            error: (prof.name || 'Cette personne') + ' est archivée — réactive-la pour la planifier.' } };
    }
    return { staff: prof };
}

// F-14 — le pendant en masse, pour les routes qui traitent une semaine entière. Un
// `findOne` par shift ferait 40 allers-retours sur une copie de semaine ; une requête
// suffit. Rend l'ensemble des `staff_id` archivés parmi ceux passés — les appelants
// en font ce que leur geste demande (la copie convertit en Joker, le push écarte).
async function archivedIdsAmong(staffIds) {
    const ids = [...new Set((staffIds || [])
        .map(id => id == null ? '' : String(id))
        .filter(id => id && id !== '__joker__' && isValidObjectId(id)))];
    if (!ids.length) return new Set();
    const docs = await db.collection('staff').find(
        { _id: { $in: ids.map(id => new ObjectId(id)) }, archived: true },
        { projection: { _id: 1 } }
    ).toArray();
...
async function sendPushToStaff(staffIds, payload) {
    // F-14 — le filtre est ICI et pas chez les appelants, parce que c'est la porte unique :
    // tout push destiné au staff passe par cette fonction, et les deux fuites relevées
    // (sollicitation Joker, publication de planning) n'étaient que deux appelants sur huit.
    // Une personne archivée ne peut plus se connecter : sans ce filtre elle reçoit des
    // sollicitations sur son téléphone SANS AUCUN MOYEN DE S'EN DÉFAIRE.
    // La notif in-app tombe avec le push, volontairement : elle ne serait jamais lue, et
    // elle s'accumulerait indéfiniment dans une boîte que personne ne peut plus ouvrir.
...
app.post('/auth/login', checkDB, async (req, res) => {
    ...
    try {
        ...
        // F-13 — un profil archivé ferme l'accès. Contrôlé APRÈS le mot de passe pour ne
        // pas révéler à un inconnu quels comptes existent.
        //
        // La source de vérité est le profil `staff`, pas un second booléen sur `users` :
        // `users.active` ressemble à un drapeau de désactivation mais signifie « invitation
        // en attente » (posé à false à la création, passé à true quand la personne choisit
...
app.delete('/api/users/:id', checkDB, requirePatron, async (req, res) => {
    ...
    try {
        ...
        if (doomed && doomed.staff_id) {
            ...
            // ⚠️ Le profil `staff` est toujours CONSERVÉ : les shifts passés, le pointage et
            // la masse salariale le référencent, et le supprimer ferait perdre son nom dans
            // tous les récaps déjà édités.
            // F-14 — mais il est désormais ARCHIVÉ. Jusqu'ici il restait `actif` : la
            // personne n'avait plus de compte et continuait pourtant d'apparaître dans la
            // barre du personnel, planifiable, notifiable. Le commentaire d'origine renvoyait
            // la décision à F-13 ; F-13 est livré, et l'archivage est exactement l'état
            // « plus dans la vie courante, tout le passé conservé » que ce cas demande.
            // `archived_at` seulement s'il ne l'était pas déjà — réarchiver ne doit pas
            // réécrire la date d'un départ plus ancien.
            if (isValidObjectId(doomed.staff_id)) {
                await db.collection('staff').updateOne(
                    { _id: new ObjectId(String(doomed.staff_id)), archived: { $ne: true } },
                    { $set: { archived: true, archived_at: new Date() } }
                ).catch(e => console.error('[delete user > archive staff]', e.message));
            }
        }
...
app.get('/api/staff', checkDB, requireAuth, async (req, res) => {
    try {
        ...
        //
        // F-13 — les archivés sont TOUJOURS renvoyés, avec leur drapeau. Les filtrer ici
        // ferait disparaître leur nom des plannings passés, du pointage et des récaps
        // déjà édités — exactement l'historique que l'archivage est censé préserver.
        // C'est à l'affichage de les écarter de la barre du personnel, pas à l'API.
...
// F-13 — Archiver : la sortie NON destructive. Demandée le 2026-08-05, sens confirmé le
// 2026-08-11 (« en gardant leurs heures »). Jusqu'ici la seule façon de sortir quelqu'un
// était le DELETE ci-dessous, qui efface ses shifts — donc son historique de paie. Un
// patron qui veut simplement dire « elle ne travaille plus ici » n'avait pas d'outil.
//
// Ce que ça change : la personne quitte la barre du planning, les rappels, la file de
// dispos, les KPI, et ne peut plus se connecter. Ce que ça ne change PAS : ses shifts
// passés, son pointage, les récaps mensuels déjà édités et la masse salariale historique.
// Réversible : réactiver rend tout, le profil n'ayant jamais été touché.
//
// Ses shifts FUTURS déjà placés sont laissés en place (décision du 2026-08-11) : archiver
// quelqu'un ne doit jamais trouer un planning déjà annoncé à l'équipe. Le compteur renvoyé
// (`upcoming_shifts`) est là pour que le patron sache ce qu'il lui reste à retirer.
app.patch('/api/staff/:id/archive', checkDB, requirePatron, async (req, res) => {
    if (!isValidObjectId(req.params.id)) return res.status(400).json({ error: 'ID invalide' });
    if (typeof req.body.archived !== 'boolean')
        return res.status(400).json({ error: 'archived (booléen) requis' });
    ...
    try {
        ...
        const r = await db.collection('staff').updateOne({ _id: new ObjectId(staffId) }, update);
        if (r.matchedCount === 0) return res.status(404).json({ error: 'Staff introuvable' });

        // La coupure d'accès passe par les sessions ET par le login (cf. /auth/login, qui
...
app.post('/api/copy-day', checkDB, requirePatron, denyObservateurEdit, async (req, res) => {
    ...
    if (!canAccessEstablishment(req.session.user, establishment_id))
        return res.status(403).json({ error: 'Accès refusé à cet établissement' });
    try {
        // F-14 — cf. `POST /api/copy-week` : un archivé ne repart pas dans une journée
        // copiée, mais son créneau ne disparaît pas pour autant — il devient un Joker.
        const archivedIds = await archivedIdsAmong(shifts.map(s => s.staff_id));
        let created = 0, jokerised = 0;
        for (const date of to_dates) {
            await db.collection('shifts').deleteMany({ establishment_id, date });
            const newShifts = shifts.map(({ _id, ...rest }) => {
        ...
        res.json({ message: created + ' shifts copiés sur ' + to_dates.length + ' jour(s)'
            + (jokerised ? ' · ' + jokerised + ' créneau(x) passé(s) en Joker (personne archivée)' : '') });
        touchLastUpdated();
    } catch (e) { console.error('[' + req.method + ' ' + req.path + ']', e); res.status(500).json({ error: 'Erreur interne' }); }
});
...
app.post('/api/copy-week', checkDB, requirePatron, denyObservateurEdit, async (req, res) => {
    ...
    try {
        ...
        // F-14 — c'était le trou le plus visible : copier la semaine faisait revenir EN MASSE
        // une personne archivée dans un planning à venir, publication et push compris, sans
        // que rien ne l'annonce. On ne supprime pas son créneau pour autant — le poste était
        // tenu, il reste à pourvoir. Il devient donc un Joker, c'est-à-dire exactement le
        // `mode: 'jokers'` que la route sait déjà faire, appliqué aux seuls partis.
        const archivedIds = await archivedIdsAmong(sourceShifts.map(s => s.staff_id));

        let created = 0, weeks = 0, jokerised = 0;
        ...
        res.json({ message: created + ' shift(s) copiés sur ' + weeks + ' semaine(s)'
            + (copyMode === 'jokers' ? ' (en Jokers)' : '')
            + (jokerised ? ' · ' + jokerised + ' créneau(x) passé(s) en Joker (personne archivée)' : '') });
        touchLastUpdated();
    } catch (e) { console.error('[' + req.method + ' ' + req.path + ']', e); res.status(500).json({ error: 'Erreur interne' }); }
});
...
app.get('/api/dispos/pending', checkDB, requirePatron, async (req, res) => {
    ...
    try {
        ...
        const mgrIds = staffIds.length
            ? new Set((await db.collection('users')
                .find({ role: 'directeur', staff_id: { $in: staffIds } }, { projection: { staff_id: 1 } })
                .toArray()).map(u => u.staff_id))
            : new Set();
        res.json(mgrIds.size ? dispos.map(d => (mgrIds.has(d.staff_id) ? { ...d, is_directeur: true } : d)) : dispos);
    } catch (e) { console.error('[' + req.method + ' ' + req.path + ']', e); res.status(500).json({ error: 'Erreur interne' }); }
});
    ...
    try {
        ...
        const eligible = activeIds.length
            ? await db.collection('staff').find(
                { _id: { $in: activeIds.map(id => new ObjectId(id)) }, can_submit_dispos: { $ne: false }, ...NOT_ARCHIVED },
...
app.patch('/api/dispos/:id/confirm', checkDB, requirePatron, denyObservateurEdit, async (req, res) => {
    ...
    try {
        ...
        if (!isOff && !canAccessEstablishment(req.session.user, establishment_id))
            return res.status(403).json({ error: 'Accès refusé à cet établissement' });
        // F-14 — refuser AVANT d'écrire le statut : confirmer avec `create_shift` planifie
        // pour de bon, et le contrôle ne vivait que dans `POST /api/shifts` — ce chemin-ci
        // recréait donc un shift pour une personne archivée sans rien dire.
        // Portée volontairement étroite : on ne bloque QUE la création de shift. Confirmer
        // sans cocher la case n'écrit aucun `staff_id` dans `shifts` et laisse au patron le
        // moyen de vider une dispo restée en file depuis avant l'archivage.
        const resolvedDispo = create_shift && !isOff
            ? await resolveStaffForPlanning({ staff_id: dispo.staff_id })
            : { staff: null };
        if (resolvedDispo.denied)
            return res.status(resolvedDispo.denied.status).json({ error: resolvedDispo.denied.error });
        ...
        if (create_shift && !isOff) {
            ...
            if (!existingShift) {
                ...
                // pour la seule couleur était un aller-retour Mongo pour rien.
                await db.collection('shifts').insertOne({
                    ...
                    color: resolvedDispo.staff?.color || '#3498db',
                });
                shiftCreated = true;
            }
        }
...
app.post('/api/me/manager-off', checkDB, requireDirecteur, async (req, res) => {
    ...
    try {
        ...
        // Symétrique du filtre congés de POST /api/dispos : les dispos déjà posées sur
        // la période (saisies ou pré-remplies par la semaine-type) sont retirées, sinon
...
// Le directeur a un profil staff (Modèle A) et passe par le pipeline `availabilities`
// STANDARD : mêmes dispos que le staff, même file d'attente, VALIDÉES PAR LE PATRON
...
//    des shifts (`source:'manager_dispo'`) : le cron réécrivait alors le planning du
//    patron chaque nuit. La dispo est une INDICATION, le shift reste la décision du
//    patron — d'où le retour au pipeline standard, sans marqueur ni chemin parallèle.
//  • Les absences du directeur restent dans `manager_time_off` (E-19, keyé user_id) :
...
// user_id y est stocké tel qu'il est en session : on tolère les deux formes.
...
//   created_at, staff_accepted_at, decided_at, decided_by, rejected_by }
//
// DEUX validations, dans cet ordre — `pending_staff` → `pending` → `approved` :
//   1. le COLLÈGUE dont le shift est convoité accepte ou refuse (il est le premier
//      concerné : sans son accord, la demande n'a rien à faire chez le patron) ;
//   2. le PATRON tranche ensuite, et lui seul déplace les shifts.
// Un refus à l'étape 1 clôt la demande (`rejected`, `rejected_by: 'staff'`).
// Le patron ne voit QUE le statut `pending` : ses routes (`/pending`, `/count`,
// `/approve`, `/reject`) filtrent déjà là-dessus, une demande encore chez le
// collègue lui reste donc invisible sans qu'elles aient à le savoir.
// Échange autorisé entre établissements différents SI le patron le permet
// (`settings.swaps.cross_establishment`). Validation patron requise dans tous les cas.

// Réglage patron : un échange peut-il traverser deux établissements ?
// Défaut `true` — c'est le comportement livré et accepté par le client, un réglage
// absent ne doit donc rien changer. Le réglage est GLOBAL : un échange est une PAIRE,
// l'autoriser d'un côté et l'interdire de l'autre ne veut rien dire.
async function swapsAllowCross() {
    const s = await db.collection('settings').findOne({ key: 'swaps' });
    return s?.cross_establishment !== false;
}
...
app.patch('/api/swap-settings', checkDB, requirePatronOnly, async (req, res) => {
    ...
    try {
        await db.collection('settings').updateOne(
            { key: 'swaps' },
            { $set: { key: 'swaps', cross_establishment } },
            { upsert: true }
        );
        res.json({ message: 'Paramètres échanges mis à jour' });
        touchLastUpdated();
    } catch (e) { console.error('[' + req.method + ' ' + req.path + ']', e); res.status(500).json({ error: 'Erreur interne' }); }
});
...
app.post('/api/shift-swaps', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        ...
        // Réglage patron. La garde tient ICI, au point d'écriture : `shifts-for-swap`
        // ne fait que peupler l'écran, il ne protège pas la route.
        if (fromShift.establishment_id !== toShift.establishment_id && !(await swapsAllowCross())) {
            return res.status(403).json({ error: 'Les échanges entre établissements sont désactivés' });
        }
        ...
        const existing = await db.collection('shift_swaps').findOne({
            // Les DEUX étapes bloquent : une demande chez le collègue engage déjà le shift.
            status: { $in: ['pending_staff', 'pending'] },
            $or: [
        ...
        // Étape 1 : c'est le COLLÈGUE qu'on prévient, pas le patron. Tant qu'il n'a pas
        // répondu, la demande n'existe pas pour le patron — c'est `/staff-accept` qui
        // pose la notification in-app patron, avec le même message qu'avant.
        res.status(201).json({
            message:  'Demande envoyée à ' + (toShift.staff_name || 'votre collègue'),
            swap_id:  String(result.insertedId),
        });
        touchLastUpdated();

        (async () => {
            try {
                await sendPushToStaff([String(toShift.staff_id)], {
                    title:   'Proposition d\'échange',
                    body:    (user.name || 'Un collègue') + ' voudrait échanger son service du ' +
                             formatDateFR(fromShift.date) + ' contre le vôtre du ' + formatDateFR(toShift.date) + '.',
                    tag:     'shift-echange',
                    url:     '/planning.html',
                    actions: [{ action: 'voir', title: 'Répondre' }],
                });
            } catch {}
        })();
    } catch (e) { console.error('[' + req.method + ' ' + req.path + ']', e); res.status(500).json({ error: 'Erreur interne' }); }
...
app.patch('/api/shift-swaps/:id/staff-accept', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (swap.status !== 'pending_staff') return res.status(409).json({ error: 'Demande déjà traitée' });
        if (String(swap.to_staff_id) !== String(staffId)) {
            return res.status(403).json({ error: 'Seul le collègue concerné peut répondre' });
        }

        // Le temps que le collègue réponde, le patron a pu retoucher le planning. Faire
        // remonter la demande sur des shifts périmés enverrait le patron valider un
        // échange qui ne correspond plus à rien — mieux vaut la clore ici.
        const [fromShift, toShift] = await Promise.all([
        ...
        const stale = !fromShift || !toShift
            || String(fromShift.staff_id) !== String(swap.from_staff_id)
            || String(toShift.staff_id)   !== String(swap.to_staff_id);
        if (stale) {
            await db.collection('shift_swaps').updateOne({ _id: swap._id }, { $set: {
                status: 'rejected', rejected_by: 'system', decided_at: new Date(),
                reject_reason: 'Le planning a changé depuis la demande',
            }});
            return res.status(410).json({ error: 'Le planning a changé depuis la demande, elle est annulée' });
        }

        await db.collection('shift_swaps').updateOne({ _id: swap._id }, { $set: {
            status: 'pending', staff_accepted_at: new Date(),
        }});

        res.json({ message: 'Échange accepté — en attente du patron' });
        touchLastUpdated();

        // Notif in-app patrons des deux établissements : elle vivait au POST, elle vit
        // maintenant ICI. Un patron ne doit être dérangé que pour ce qu'il peut trancher.
        const message = (swap.from_staff_name || 'Un staff') + ' propose un échange : ' +
            ...
            ' (accepté par ' + (toShift.staff_name || 'le collègue') + ')';
        await createNotifForPatrons(fromShift.establishment_id, 'shift_swap_request', message, { swap_id: String(swap._id) });
        if (toShift.establishment_id !== fromShift.establishment_id) {
            await createNotifForPatrons(toShift.establishment_id, 'shift_swap_request', message, { swap_id: String(swap._id) });
        }

        (async () => {
            try {
                await sendPushToStaff([String(swap.from_staff_id)], {
                    title:   'Échange accepté par votre collègue',
                    body:    (swap.to_staff_name || 'Votre collègue') + ' a accepté. En attente de la validation du patron.',
                    tag:     'shift-echange',
                    url:     '/planning.html',
                });
            } catch {}
...
app.patch('/api/shift-swaps/:id/staff-decline', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        ...
        await db.collection('shift_swaps').updateOne({ _id: swap._id }, { $set: {
            status: 'rejected', rejected_by: 'staff', decided_at: new Date(),
            decided_by: String(req.session.user._id), reject_reason: reason || null,
        }});
        ...
        (async () => {
            try {
                await sendPushToStaff([String(swap.from_staff_id)], {
                    ...
                    body:    (swap.to_staff_name || 'Votre collègue') + ' ne peut pas échanger' + (reason ? ' : ' + reason : '.'),
                    tag:     'shift-echange',
                    url:     '/planning.html',
                });
            } catch {}
...
app.get('/api/shifts-for-swap', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        ...
        const isVisible = await publishedShiftFilter();
        let estabIds = [...new Set(own.filter(isVisible).map(s => s.establishment_id))];
        // Réglage patron : sans échange inter-établissements, les cibles se limitent à
        // l'établissement du shift proposé, que le client passe en `establishment_id`.
        // Sans ce paramètre on ne restreint pas — le refus reste garanti par le POST.
        if (req.query.establishment_id && !(await swapsAllowCross())) {
            estabIds = estabIds.filter(id => id === req.query.establishment_id);
        }
        ...
        // Exclure les shifts déjà engagés — chez le collègue comme chez le patron.
...
app.delete('/api/shift-swaps/:id', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        const swap = await db.collection('shift_swaps').findOne({ _id: new ObjectId(req.params.id) });
        if (!swap) return res.status(404).json({ error: 'Demande introuvable' });
        // Annulable aux DEUX étapes : tant que le patron n'a pas tranché, le proposeur
        // reste maître de sa demande — y compris pendant que le collègue réfléchit.
        if (swap.status !== 'pending_staff' && swap.status !== 'pending') return res.status(409).json({ error: 'Demande déjà traitée' });
...
app.post('/api/shifts/extra', checkDB, requireAuth, async (req, res) => {
    ...
    try {
        // F-14 — cette route est la seule à résoudre PAR NOM, et l'autocomplete du pointage
        // proposait encore les archivés : taper les 3 premières lettres d'une personne
        // partie lui recréait un shift, avec ses heures réelles. La résolution (par id ou
        // par nom) et le refus vivent maintenant au même endroit que pour les autres portes.
        const resolved = await resolveStaffForPlanning({ staff_id, staff_name });
        if (resolved.denied)
            return res.status(resolved.denied.status).json({ error: resolved.denied.error });

        const staffDoc        = resolved.staff;
