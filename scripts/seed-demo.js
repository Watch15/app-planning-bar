...
// Mêmes collections que la recette : ce script est relançable à volonté.
const WIPE = [
...
const SERVICES = {
    [C]: dow => {
        const base = [
            { start: 17, end: 24,   resp: true },
            { start: 18, end: 26 },
        ];
        if (dow >= 4 && dow <= 5) base.push({ start: 20, end: 26 }); // renfort ven/sam
        return base;
    },
    [B]: dow => {
        if (dow === 0) return [];                                     // fermé le lundi
        return [
            { start: 11,   end: 16 },
            { start: 11.5, end: 16 },
    ...
    [R]: dow => {
        if (dow <= 1) return [];                                      // fermé lun/mar
        const base = [
            { start: 17, end: 26, resp: true },
            { start: 18, end: 26 },
        ];
        if (dow >= 4 && dow <= 5) base.push({ start: 19, end: 26 });
        return base;
    },
};
...
async function run() {
    ...
    // Garde-fou SUPPLÉMENTAIRE à celui de `_db.js`. Celui-ci ne protège que la base
    // client (`gestion_bar`) ; or ce script écrase aussi bien `templyo_dev`, et
    // perdre la recette en préparant une démo serait idiot. La base de démo doit
    // donc s'appeler quelque chose qui contient « demo ».
    if (!/demo/i.test(dbName) && !process.argv.includes('--force')) {
        console.error('\n⛔ REFUS — « ' + dbName + ' » n\'est pas une base de démo.');
        console.error('   Ce script EFFACE 15 collections. Vise une base dédiée :\n');
        console.error('     npm run demo:seed            (utilise .env.demo → MONGO_DB=templyo_demo)\n');
        console.error('   Si tu sais ce que tu fais, relance avec --force.\n');
        await client.close();
        process.exit(1);
    }
    ...
    try {
        ...
        // ── Établissements ────────────────────────────────────────────────────
        await db.collection('establishments').insertMany(ESTABS.map(e => ({ ...e })));
        ...
        staffDefs.forEach((s, i) => { sid[s.name] = String(staffIns.insertedIds[i]); });
        const byId = {};  // _id → définition, pour retrouver taux/couleur à l'affectation
        staffDefs.forEach(s => { byId[sid[s.name]] = s; });
        ...
        // NFD sépare « é » en « e » + accent combinant, que la plage ̀-ͯ retire :
        // Chloé → chloe, Gaëlle → gaelle, Loïc → loic. Une adresse accentuée serait
        // pénible à dicter en visio et refusée par certains clients mail.
        const slug = n => n.split(' ')[0].normalize('NFD')
        ...
        const uid = {};
        accounts.forEach((a, i) => { uid[a.name] = String(userIns.insertedIds[i]); });
        ...
        await db.collection('time_off').insertMany(conges.map(c => ({
        ...
        const eligible = {};          // estab → [staff_id] pouvant y travailler
        ESTABS.forEach(e => {
            eligible[e.id] = staffDefs.filter(s => s.venues.includes(e.id)).map(s => sid[s.name]);
        });
        // Heures cumulées par personne, pour affecter le MOINS CHARGÉ à chaque créneau.
        // Un tirage purement aléatoire donnait des semaines à 38 h pour l'un et 4 h pour
        // l'autre : un planning que personne ne reconnaîtrait, et le premier écran que
        // le prospect regarde. L'équilibrage est donc une exigence de crédibilité.
        const hoursSoFar = {};
        staffDefs.forEach(s => { hoursSoFar[sid[s.name]] = 0; });
        ...
        for (let d = new Date(histStart); d <= nextSun; d = addDays(d, 1)) {
            ...
            for (const estab of ESTABS) {
                const slots = SERVICES[estab.id](dow);
                if (!slots.length) continue;

                // Pool du jour : éligible, libre, pas en congé.
                const pool = eligible[estab.id]
                    .filter(id => !busyToday.has(id) && !blocked.has(id + '|' + date));
                // L'extra n'est appelé qu'en renfort de fin de semaine, pas tous les jours —
                // et comme il est structurellement le moins chargé, il serait sinon pris
                // à chaque créneau par la règle d'équilibrage.
                const usable = pool.filter(id => byId[id].extra !== true || (dow >= 4 && rnd() < 0.2));

                for (const slot of slots) {
                    const free = usable.filter(id => !busyToday.has(id));
                    ...
                    let candidates = free;
                    if (slot.resp) {
                        const resps = free.filter(id => byId[id].roles.includes(RESP));
                        if (resps.length) candidates = resps;
                    }
                    // Score calculé UNE fois par candidat : appeler le PRNG dans le
                    // comparateur rendrait l'ordre incohérent d'une comparaison à l'autre.
                    // Le bruit ±1 h évite une rotation rigoureusement identique chaque semaine.
                    const score = new Map(candidates.map(c => [c, hoursSoFar[c] + rnd() * 2 - 1]));
                    const id = candidates.reduce((a, b) => (score.get(b) < score.get(a) ? b : a));
                    busyToday.add(id);
                    hoursSoFar[id] += slot.end - slot.start;

                    const def = byId[id];
                    const shift = {
                        establishment_id: estab.id, staff_id: id, staff_name: def.name,
                    ...
                    if (past) {
                        // Pointage : l'écart planifié/réel est la valeur que Templyo révèle.
                        // Sortie plus tard qu'annoncé dans la majorité des cas — c'est ce
                        // qu'un patron reconnaît immédiatement de son propre établissement.
                        const late  = pick([0, 0, 0, 0.25, 0.25, 0.5, 0.5, 0.75, 1]);
                        const early = pick([0, 0, 0, 0, 0.25]);
                        shift.real_start = round2(slot.start + early);
                        shift.real_end   = round2(slot.end + late);
                        ...
                        const wage = def.fixed != null
                            ? def.fixed
                            : (shift.real_end - shift.real_start) * def.rate;
                        const k = estab.id + '|' + date;
                        wageByEstabDate[k] = (wageByEstabDate[k] || 0) + wage;
                    }
                    shifts.push(shift);
                }
            }
        ...
        shifts.push({
            ...
            joker_candidates: [
                { staff_id: sid['Jonas Weber'], staff_name: 'Jonas Weber', staff_color: '#2980b9', submitted_at: addDays(now, -2) },
                { staff_id: sid['Gaëlle Nunès'], staff_name: 'Gaëlle Nunès', staff_color: '#f39c12', submitted_at: addDays(now, -1) },
            ],
        });
        ...
        const revenues = Object.entries(wageByEstabDate).map(([k, gross]) => {
            ...
            const revenue = (gross * 1.45) / (target / 100);
            return { establishment_id, date, revenue: Math.round(revenue / 10) * 10 };
        });
        await db.collection('daily_revenue').insertMany(revenues);
        ...
        await db.collection('availabilities').insertMany(dispos);
        ...
        await db.collection('manager_time_off').insertOne({
            user_id: uid[DIRECTOR.name], name: DIRECTOR.name,
        ...
        const settings = [
            ...
            { key: 'performance',                 target_charged: 31, charge_rate: 45 },
            { key: 'performance_' + C,            target_charged: 31, charge_rate: 45 },
            { key: 'performance_' + B,            target_charged: 32, charge_rate: 45 },
            { key: 'performance_' + R,            target_charged: 30, charge_rate: 45 },
        ...
        await db.collection('notifications').insertOne({
            type: 'rappel_dispo',
            message: 'Rappel dispos envoyé à 11 membre(s) — semaine du ' + toDateStr(nextMon),
            week_start: toDateStr(nextMon),
            sent_to: staffDefs.map(s => sid[s.name]),
